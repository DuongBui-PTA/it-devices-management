...
def allocate_device(device_id: int, employee_id: int, allocated_by: int, note: str = "") -> bool:
    try:
        engine = get_db_engine()
        ...
        with engine.begin() as conn:
            ...
            conn.execute(update_device_status, {"device_id": device_id})

        return True

    except Exception as e:
        logger.error(e)
        st.error("Không thể cấp phát thiết bị.")
        return False
...
def return_device(device_id: int) -> bool:
    try:
        ...
        with engine.begin() as conn:
            affected = conn.execute(close_alloc, {"device_id": device_id}).rowcount
            if affected == 0:
                st.warning("⚠️ Thiết bị chưa được cấp phát.")
                return False
        ...
        logger.error(e)
        st.error("Không thể thu hồi thiết bị.")
        return False